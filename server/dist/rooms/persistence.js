...
export async function loadRoomSnapshot(roomId) {
    ...
    try {
        ...
        const gameState = (rawGameState && Object.keys(rawGameState).length > 0) ? rawGameState : undefined;
        const recoveryMetadata = data.recovery_metadata;
        const room = {
            roomId: data.room_id,
            ...
            status: data.status,
            players: (recoveryMetadata?.players || []).map((p) => ({
                ...p,
            ...
            reconnectMetadata: data.reconnect_metadata,
        };
...
export async function loadAllActiveRooms() {
    if (!supabase)
        return [];
    try {
        ...
        for (const record of data) {
            try {
                ...
                const gameState = (rawGameState && Object.keys(rawGameState).length > 0) ? rawGameState : undefined;
                const recoveryMetadata = record.recovery_metadata;
                // RECOVERY RULES:
                ...
                const players = (recoveryMetadata?.players || []).map((p) => ({
                    ...p,
                    socketId: undefined,
                    status: 'disconnected',
                }));
                const room = {
                    ...
                    reconnectMetadata: record.reconnect_metadata,
                    recoveryMetadata: {
                        ...recoveryMetadata,
                        restoredAt: Date.now(),
                        restartRecoveryId,
                        recoveryReason: 'backend_restart',
                        awaitingReconnectPlayers: players.map((p) => p.playerId),
                    },
                };
                rooms.push(room);
