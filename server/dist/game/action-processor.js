...
import { createGameStateSnapshot, createSuccessAcknowledgement, createFailureAcknowledgement } from './synchronization.js';
...
export class ActionProcessor {
    ...
    processAction(action, gameState) {
        ...
        if (gameState.gameEnded) {
            return {
                success: false,
                acknowledgement: createFailureAcknowledgement(gameState.version, 'Game has already ended. No further actions allowed.'),
                error: {
                    code: 'GAME_ENDED',
        ...
        if (!isActionVersionValid(action.clientVersion, gameState.version, this.config.versionTolerance)) {
            return {
                success: false,
                acknowledgement: createFailureAcknowledgement(gameState.version, `Version mismatch. Client: ${action.clientVersion}, Server: ${gameState.version}`),
                error: {
        ...
        const nextState = result.gameState ?? result.state;
        if (!nextState) {
            return {
                success: false,
        ...
        if (!nextState.gameEnded) {
            ...
            if (winnerId) {
                ...
                // Log winner
                const winnerPlayer = nextState.players.find((p) => p.id === winnerId);
        ...
        if (nextState.winner)
            console.log(`WINNER ANNOUNCED: ${nextState.winner}`);
        console.log('------------------------');
 ...
 */
export function createActionProcessor(config) {
