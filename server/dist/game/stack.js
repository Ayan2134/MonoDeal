...
export function isResponseEligibleAction(actionId) {
...
export function toStackSnapshot(state) {
    return {
        roomId: state.roomId,
...
export function enqueuePendingAction(state, actorId, card, targets, now = Date.now()) {
    const actionEntry = {
        id: randomUUID(),
        kind: 'action',
        actorId,
...
export function respondWithJustSayNo(state, playerId, justSayNoCardId, targetEntryId, now = Date.now()) {
...
function isEntryCountered(entryId, stack, memo = new Map()) {
    if (memo.has(entryId)) {
...
export function resolvePendingStack(state) {
