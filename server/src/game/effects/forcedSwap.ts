...
export const forcedSwapEffect: EffectHandler = (context) => {
  ...
  let initiatorCardId: string | undefined;
  ...
  for (const cardId of propertyCardIds) {
    ...
    if (isInInitiator) {
      if (initiatorCardId) return { ok: false, error: 'You selected more than one of your own properties.' };
      initiatorCardId = cardId;
      
      // Check if it's in a complete set
      const set = initiator.properties.find(s => s.cards.some(c => c.id === cardId));
      if (set?.isComplete) return { ok: false, error: 'You cannot swap a card from a complete set.' };
    } else if (isInTarget) {
      if (targetCardId) return { ok: false, error: 'You selected more than one of the target\'s properties.' };
      targetCardId = cardId;

      // Check if it's in a complete set
      const set = targetPlayer.properties.find(s => s.cards.some(c => c.id === cardId));
      if (set?.isComplete) return { ok: false, error: 'You cannot take a card from a complete set.' };
    } else {
  ...
  const newInteraction = {
    ...
    interactionType: 'forced-swap' as const,
